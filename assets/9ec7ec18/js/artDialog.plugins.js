 ...
 */
$.tips = $.dialog.tips = function (content, time, options) {
	options = $.extend({
        id: 'Tips',
        title: false,
        cancel: false,
	...
	config = artDialog(options)
    .content('<div style="padding: 0 1em;">' + content + '</div>');
	if(time != -1) config.time(time || 1500);
    return config;
};

$.notice = $.dialog.notice = function (options) {
    var opt = options || {},
...
$.error = $.dialog.error = function(title, content, callback) {
	return $.dialog({
        id: 'Error',
        title: title,
        fixed: true,
        lock: true,
        content: _wrapContent(content, 'error'),
        ok: true,
        beforeunload: callback
    });
}

$.warning = $.dialog.warning = function(title, content, callback) {
	return $.dialog({
        id: 'Warning',
        title: title,
        fixed: true,
        lock: true,
        content: _wrapContent(content, 'warning'),
        ok: true,
        beforeunload: callback
    });
}

$.confirmation = $.dialog.confirmation = function(title, content, callback) {
	return $.dialog({
        id: 'Confirmation',
        title: title,
        fixed: true,
        lock: true,
        content: _wrapContent(content, 'confirmation'),
        ok: true,
        beforeunload: callback
    });
}

$.information = $.dialog.information = function(title, content, callback) {
	return $.dialog({
        id: 'Information',
        title: title,
        fixed: true,
        lock: true,
        content: _wrapContent(content, 'information'),
        ok: true,
        beforeunload: callback
    });
}
...
$.question = $.dialog.question = function(title, content, callback) {
	return $.dialog({
        id: 'Question',
        title: title,
        fixed: true,
        lock: true,
        content: _wrapContent(content, 'question'),
        ok: true,
        beforeunload: callback
    });
}

function _wrapContent(content, type) {
	if(typeof content == 'object') {
		$content = $(content);
		if($content.hasClass('d-content-icon'))
			$(content).children().unwrap()
		
		return $(content).wrap('<div class="d-content-icon d-content-'+type+'"></div>');
	} else {
		return '<div class="d-content-icon d-content-'+type+'">'+content+'</div>';
	}
}
