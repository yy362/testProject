...
artDialog.fn = artDialog.prototype = {
    ...
    */
    follow: function (elem) {
        ...
        if(config.offset.top) {
        	setTop += config.offset.top;
        }
        if(config.offset.left) {
        	setLeft += config.offset.left;
        }
        style.left = setLeft + 'px';
        style.top = setTop-1 + 'px';
    ...
    */
    ...
    toogleSize: function() {
    	dom = this.dom;
    	minimize = dom.minimize;
    	if(minimize.hasClass('isMinimize')) {
    		dom.main.show();
        	dom.footer.show();
        	minimize.removeClass('isMinimize');
        	minimize.text('-').attr('title', '缩小窗口');
    	} else {
    		dom.main.hide();
        	dom.footer.hide();
    	    minimize.addClass('isMinimize');
    	    minimize.text('+').attr('title', '还原窗口');
    	}
...
+										'<a class="d-close" title="关闭" href="javascript:/*artDialog*/;">'
+											'\xd7'
+										'</a>'
//+										'<a class="d-minimize" title="最小化" href="javascript:/*artDialog*/;">'
//+											'-'
//+										'</a>'
+									'</div>'
+								'</td>'
